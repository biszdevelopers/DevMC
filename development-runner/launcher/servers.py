...
def find_java25() -> str:
    candidates = [
        ...
        Path(r"C:\Program Files\Eclipse Adoptium\jdk-25.0.0.0-hotspot\bin\java.exe"),
    ]
...
def profiles(data_dir: Path) -> dict[str, ServerProfile]:
    ...
    return {
        "mohist-1.20.1": ServerProfile(
            "mohist-1.20.1", "Mohist 1.20.1", "mohist", "1.20.1",
            server_root / "mohist-1.20.1", 17,
        ),
        "spigot-1.20.1": ServerProfile(
            "spigot-1.20.1", "Spigot 1.20.1", "spigot", "1.20.1",
            server_root / "spigot-1.20.1", 17,
        ),
        "paper-26.2": ServerProfile(
            "paper-26.2", "Paper 26.2", "paper", "26.2",
            server_root / "paper-26.2", 25,
        ),
        "youer-26.2": ServerProfile(
            "youer-26.2", "Youer 26.2", "youer", "26.2",
            server_root / "youer-26.2", 25,
        ),
    }
...
def validate_paper_jar(path: Path) -> None:
    if not path.is_file():
        raise FileNotFoundError(f"Paper server jar was not found: {path}")
    try:
        with zipfile.ZipFile(path) as archive:
            manifest = archive.read("META-INF/MANIFEST.MF").decode("utf-8", errors="replace")
            version = json.loads(archive.read("version.json"))
    except (OSError, KeyError, ValueError, zipfile.BadZipFile) as error:
        raise RuntimeError(f"Invalid Paper server jar: {path}") from error
    is_paper = "Main-Class: io.papermc.paperclip.Main" in manifest
    minecraft_version = str(version.get("id", ""))
    if not is_paper or not (minecraft_version == "26.2" or minecraft_version.startswith("26.2.")):
        raise RuntimeError("The selected jar is not a Paper 26.2 server jar")


def install_paper(profile: ServerProfile, source: Path) -> None:
    validate_paper_jar(source)
    _install_server_jar(profile, source)


def validate_youer_jar(path: Path) -> None:
    if not path.is_file():
        raise FileNotFoundError(f"Youer server jar was not found: {path}")
    try:
        with zipfile.ZipFile(path) as archive:
            names = set(archive.namelist())
    except (OSError, zipfile.BadZipFile) as error:
        raise RuntimeError(f"Invalid Youer server jar: {path}") from error
    launcher = "com/mohistmc/launcher/youer/Main.class"
    if launcher not in names:
        raise RuntimeError("The selected jar is not a Youer 26.2 server jar")


def install_youer(profile: ServerProfile, source: Path) -> None:
    validate_youer_jar(source)
    _install_server_jar(profile, source)
...
def validate_java(java_path: str, required_major: int) -> None:
    ...
    match = re.search(r'(?:openjdk version|java version) "(?P<major>\d+)', first_line)
    if result.returncode or match is None or int(match.group("major")) != required_major:
        raise RuntimeError(
            f"This server profile requires Java {required_major}; found: {first_line or 'unknown'}"
        )
