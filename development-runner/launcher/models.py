...
class ServerProfile:
    profile_id: str
    display_name: str
    kind: Literal["mohist", "spigot", "paper", "youer"]
    version: str
    runtime_dir: Path
    required_java: int
...
class LauncherSettings:
    ...
    active_profile: str = "paper-26.2"
    mohist_source: str = str(Path.home() / "Downloads" / "server.jar")
    spigot_source: str = str(Path.home() / "Downloads" / "spigot-1.20.1.jar")
    paper_source: str = str(Path.home() / "Downloads" / "paper-26.2-124.jar")
    youer_source: str = str(Path.home() / "Downloads" / "youer-26.2-server.jar")
    java_path: str = ""
