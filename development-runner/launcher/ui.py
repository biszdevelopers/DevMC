...
class LauncherApp:
    ...
    def _create_variables(self) -> None:
        ...
        self.paper_source_var = tk.StringVar(value=self.settings.paper_source)
        self.youer_source_var = tk.StringVar(value=self.settings.youer_source)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(outer, text="Build, deploy, configure, and run Paper/Youer 26.2 or legacy 1.20.1 development servers.").pack(anchor="w", pady=(0, 10))
    ...
    def _build_server_tab(self) -> None:
        ...
        ttk.Label(tab, text="Paper 26.2 source jar").grid(row=4, column=0, sticky="w", pady=4)
        self.paper_entry = ttk.Entry(tab, textvariable=self.paper_source_var)
        self.paper_entry.grid(row=4, column=1, columnspan=2, sticky="ew", padx=8)
        self.paper_browse = ttk.Button(tab, text="Browse...", command=self._browse_paper)
        self.paper_browse.grid(row=4, column=3)
        ttk.Label(tab, text="Youer 26.2 source jar").grid(row=5, column=0, sticky="w", pady=4)
        self.youer_entry = ttk.Entry(tab, textvariable=self.youer_source_var)
        self.youer_entry.grid(row=5, column=1, columnspan=2, sticky="ew", padx=8)
        self.youer_browse = ttk.Button(tab, text="Browse...", command=self._browse_youer)
        self.youer_browse.grid(row=5, column=3)
        ttk.Label(tab, text="Java executable").grid(row=6, column=0, sticky="w", pady=4)
        ttk.Entry(tab, textvariable=self.java_var).grid(row=6, column=1, columnspan=2, sticky="ew", padx=8)
        ttk.Button(tab, text="Browse...", command=self._browse_java).grid(row=6, column=3)
        self.profile_status = ttk.Label(tab)
        self.profile_status.grid(row=7, column=0, columnspan=3, sticky="w", pady=6)
        self.spigot_button = ttk.Button(tab, text="Import/Reimport Spigot", command=self.import_spigot_now)
        self.spigot_button.grid(row=7, column=3)
        self.paper_button = ttk.Button(tab, text="Import/Reimport Paper", command=self.import_paper_now)
        self.paper_button.grid(row=7, column=3)
        self.youer_button = ttk.Button(tab, text="Import/Reimport Youer", command=self.import_youer_now)
        self.youer_button.grid(row=7, column=3)

        memory = ttk.LabelFrame(tab, text="Memory & JVM tweaks", padding=10)
        memory.grid(row=8, column=0, columnspan=4, sticky="ew", pady=10)
        ...
        debug.grid(row=9, column=0, columnspan=4, sticky="ew", pady=(0, 10))
        ...
        authentication.grid(row=10, column=0, columnspan=4, sticky="ew", pady=(0, 10))
        ...
        dimensions.grid(row=11, column=0, columnspan=4, sticky="ew", pady=(0, 10))
        ttk.Checkbutton(
        ...
        eula_frame.grid(row=12, column=0, columnspan=4, sticky="ew", pady=6)
        ...
        ttk.Label(tab, text="Java run command (read-only)", style="Heading.TLabel").grid(row=13, column=0, columnspan=4, sticky="w", pady=(12, 4))
        command_frame = ttk.Frame(tab)
        command_frame.grid(row=14, column=0, columnspan=4, sticky="ew")
        ...
        self.launch_summary.grid(row=15, column=0, columnspan=4, sticky="w", pady=(6, 0))
        actions.grid(row=16, column=0, columnspan=4, sticky="w", pady=(14, 0))
    ...
    def _build_plugins_tab(self) -> None:
        ...
        self.additional_mods_frame = ttk.Labelframe(
            self.plugins_tab, text="Additional hybrid-server test mods", padding=8,
        )
        self.additional_mods_frame.pack(fill="x", pady=(10, 0))
        ttk.Label(
            self.additional_mods_frame,
            text="Test mod JARs are deployed only to the selected Mohist or Youer profile's mods folder.",
        ).pack(anchor="w", pady=(0, 6))
    ...
    def _update_profile_ui(self) -> None:
        ...
        paper_state = "normal" if profile.kind == "paper" and source_enabled else "disabled"
        self.paper_entry.configure(state=paper_state)
        self.paper_browse.configure(state=paper_state)
        self.paper_button.configure(state=paper_state)
        youer_state = "normal" if profile.kind == "youer" and source_enabled else "disabled"
        self.youer_entry.configure(state=youer_state)
        self.youer_browse.configure(state=youer_state)
        self.youer_button.configure(state=youer_state)
        if profile.kind == "paper":
            self.spigot_button.grid_remove()
            self.youer_button.grid_remove()
            self.paper_button.grid()
        elif profile.kind == "youer":
            self.spigot_button.grid_remove()
            self.paper_button.grid_remove()
            self.youer_button.grid()
        else:
            self.paper_button.grid_remove()
            self.youer_button.grid_remove()
            self.spigot_button.grid()
        ...
        if profile.kind in ("mohist", "youer"):
            self.additional_mods_frame.pack(fill="x", pady=(10, 0))
        else:
    ...
    def _browse_paper(self) -> None:
        path = filedialog.askopenfilename(title="Select Paper 26.2 server jar", filetypes=[("Java archives", "*.jar")])
        if path:
            self.paper_source_var.set(path)
            self._save_settings()

    def _browse_youer(self) -> None:
        path = filedialog.askopenfilename(title="Select Youer 26.2 server jar", filetypes=[("Java archives", "*.jar")])
        if path:
            self.youer_source_var.set(path)
            self._save_settings()

    def _browse_java(self) -> None:
        path = filedialog.askopenfilename(
            title=f"Select Java {self.profile.required_java} executable",
    ...
    def _add_additional_mods(self) -> None:
        if self.profile.kind not in ("mohist", "youer"):
            return
        selected_paths = filedialog.askopenfilenames(
            title="Select additional hybrid-server mod JARs",
            initialdir=str(Path.home() / "Downloads"),
            filetypes=(("Java archives", "*.jar"), ("All files", "*.*")),
        )
        if not selected_paths:
        ...
        for selected_path in selected_paths:
            path = Path(selected_path).expanduser().resolve()
            if path.suffix.casefold() != ".jar":
                messagebox.showerror("Additional mod", f"{path.name} is not a JAR file.")
                continue
            normalized = str(path)
            if normalized.casefold() in existing:
                continue
        ...
        self._save_settings()
        if added:
            self.status_var.set(f"Added {added} test mod JAR(s)")

    def _remove_selected_additional_mod(self) -> None:
        selected = self.additional_mod_tree.selection()
        if not selected:
            self.status_var.set("Select an additional mod JAR first")
            return
        ...
        self._render_additional_mods()
        self._save_settings()
        self.status_var.set(f"Removed {Path(path).name} from test mods")
    ...
    def _begin_build(self, run_tests: bool, launch: bool) -> None:
        ...
        def worker() -> None:
            try:
                if launch:
                    validate_java(self.java_var.get(), profile.required_java)
                    ...
                    elif profile.kind == "spigot":
                        self._queue_output("Validating and importing Spigot 1.20.1...")
                        install_spigot(profile, Path(self.spigot_source_var.get()))
                    else:
                        if profile.kind == "paper":
                            self._queue_output("Validating and importing Paper 26.2...")
                            install_paper(profile, Path(self.paper_source_var.get()))
                        else:
                            self._queue_output("Validating and importing Youer 26.2...")
                            install_youer(profile, Path(self.youer_source_var.get()))
                ...
                if launch:
                    ...
                    if profile.kind in ("mohist", "youer"):
                        ...
                        if additional_mod_artifacts:
                            self._queue_output(
                                "[launcher] Deploying additional hybrid-server mods: " + ", ".join(
    ...
    def import_paper_now(self) -> None:
        if self.busy or self.server.running:
            return
        profile = self.profiles["paper-26.2"]
        self._start_log()
        self.notebook.select(self.console_tab)
        self._set_busy(True, "Importing Paper 26.2...")

        def worker() -> None:
            try:
                install_paper(profile, Path(self.paper_source_var.get()))
                self.events.put(("operation_done", "Paper 26.2 is ready"))
            except Exception as error:
                self.events.put(("operation_error", str(error)))

        threading.Thread(target=worker, daemon=True).start()

    def import_youer_now(self) -> None:
        if self.busy or self.server.running:
            return
        profile = self.profiles["youer-26.2"]
        self._start_log()
        self.notebook.select(self.console_tab)
        self._set_busy(True, "Importing Youer 26.2...")

        def worker() -> None:
            try:
                install_youer(profile, Path(self.youer_source_var.get()))
                self.events.put(("operation_done", "Youer 26.2 is ready"))
            except Exception as error:
                self.events.put(("operation_error", str(error)))
    ...
    def _save_settings(self) -> None:
        ...
        self.settings.paper_source = self.paper_source_var.get()
        self.settings.youer_source = self.youer_source_var.get()
