...
class SettingsTests(unittest.TestCase):
    ...
    def test_paper_source_persists(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            store = SettingsStore(Path(temporary) / "settings.json")
            expected = r"D:\Downloads\paper-26.2-124.jar"
            store.save(LauncherSettings(paper_source=expected))
            self.assertEqual(expected, store.load().paper_source)

    def test_youer_source_persists(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            store = SettingsStore(Path(temporary) / "settings.json")
            expected = r"D:\Downloads\youer-26.2-server.jar"
            store.save(LauncherSettings(youer_source=expected))
            self.assertEqual(expected, store.load().youer_source)
