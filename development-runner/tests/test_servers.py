...
class ServerCommandTests(unittest.TestCase):
    ...
    def test_has_paper_and_youer_262_profiles_requiring_java25(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            available = profiles(Path(temporary))
            paper = available["paper-26.2"]
            youer = available["youer-26.2"]
            self.assertEqual(("paper", "26.2", 25), (paper.kind, paper.version, paper.required_java))
            self.assertEqual(("youer", "26.2", 25), (youer.kind, youer.version, youer.required_java))

    def test_validates_paper_262_jar(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            jar = Path(temporary) / "paper-26.2-124.jar"
            with zipfile.ZipFile(jar, "w") as archive:
                archive.writestr("META-INF/MANIFEST.MF", "Main-Class: io.papermc.paperclip.Main\n")
                archive.writestr("version.json", '{"id":"26.2","java_version":25}')
            validate_paper_jar(jar)

    def test_rejects_paper_from_another_minecraft_series(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            jar = Path(temporary) / "paper-26.1.2.jar"
            with zipfile.ZipFile(jar, "w") as archive:
                archive.writestr("META-INF/MANIFEST.MF", "Main-Class: io.papermc.paperclip.Main\n")
                archive.writestr("version.json", '{"id":"26.1.2","java_version":25}')
            with self.assertRaisesRegex(RuntimeError, "Paper 26.2"):
                validate_paper_jar(jar)

    def test_validates_youer_262_jar(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            jar = Path(temporary) / "youer-26.2-server.jar"
            with zipfile.ZipFile(jar, "w") as archive:
                archive.writestr("com/mohistmc/launcher/youer/Main.class", b"")
            validate_youer_jar(jar)
