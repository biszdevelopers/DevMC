...
Server messages live in `en_us.json`, `zh_cn.json`, `zh_tw.json`, `ja_jp.json`, `ko_kr.json`, and `es_es.json`. The corresponding `*_mojang.json` files contain the official Minecraft 1.20.1 language mappings; `zh_cn_mojang.json` is included. Missing server-message keys fall back to `zh_cn`, while missing Mojang keys fall back to `en_us_mojang`.
