...
Status: authoritative. This describes how the `worldgen` plugin shapes the
managed world. It replaces the earlier custom-terrain "rust map" generator.
...
5. `shouldGenerateDecorations()/Structures()/Mobs() = false` — the plugin's
   feature pass still owns ores, plants, animals, and POIs.
...
1. **Ocean ring.** If `falloff(x, z)` is inside the carved-ocean band, return
   `OCEAN` so the sculpted coast reads as sea.
2. **Natural water.** Otherwise, if the vanilla `continentalness` says ocean,
   return `OCEAN`/`DEEP_OCEAN`. Oceans and rivers are never moved.
