...
public final class WorldInfoCommand extends DevCommand {
  ...
  public WorldInfoCommand(WorldGenPlugin plugin) {
    super("worldinfo", "worldgen.admin");
    this.plugin = plugin;
  }
  ...
  private void makeTest(CommandSender sender, String[] arguments) {
    ...
    if (arguments.length > 3) {
      if (arguments[3].equalsIgnoreCase("tweak")) {
        creator.generator(
          new dev.bisz.worldgen.worldgen.BareboneGenerator(
        ...
        creator.generator(
          dev.bisz.worldgen.worldgen.WorldGenerators.create(plugin.settings())
        );
      } else {
        creator.generator(
          new dev.bisz.worldgen.worldgen.BareboneGenerator(1.0, false, 0.06, 0.62, 63)
        );
      }
    }
  ...
  private void biomes(CommandSender sender) {
    ...
    if (
      ...
      dev.bisz.worldgen.worldgen.IslandWorldGenerator island
    ) {
      sender.sendMessage(
        "island biomes=" +
        island
  ...
  private void chunk(CommandSender sender, String[] arguments) {
    ...
    sender.sendMessage(
      "chunk " + key.encode() + " " + (status == null ? "untracked" : status)
    );
  }
