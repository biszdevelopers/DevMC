package dev.bisz.worldgen.loot;
...
public final class LootTables {
  ...
  public static final String FILE = "worldgen/loot.json";
  ...
  public static LootTables load(JavaPlugin plugin) {
    try (InputStream defaults = plugin.getResource("worldgen/loot.json")) {
      if (defaults == null) {
        throw new IllegalStateException("Missing bundled settlement loot defaults");
      }
      JSON.mergeMissingDefaults(FILE, defaults);
    } catch (IOException exception) {
      throw new IllegalStateException(
        "Cannot close bundled settlement loot defaults",
        exception
      );
    }
