...
public final class PriceTable {
  ...
  public static final String FILE = "world/prices.json";
  ...
  public static PriceTable load(JavaPlugin plugin) {
    try (
      InputStream defaults = plugin.getResource("world/prices.json")
    ) {
      if (defaults == null) {
        throw new IllegalStateException(
          "Missing bundled settlement price defaults"
        );
      }
      JSON.mergeMissingDefaults(FILE, defaults);
    } catch (IOException exception) {
      throw new IllegalStateException(
        "Cannot close bundled settlement price defaults",
        exception
      );
    }
