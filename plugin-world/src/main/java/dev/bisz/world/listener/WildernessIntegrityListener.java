...
import dev.bisz.world.wilderness.ScratchRegenerator;
...
public final class WildernessIntegrityListener implements Listener {
  ...
  public WildernessIntegrityListener(WorldPlugin plugin) {
    this.plugin = plugin;
  }
  ...
  public void onUnload(ChunkUnloadEvent event) {
    ...
    if (!plugin.settlements().isManagedWorld(key.world())) return;
    if (plugin.settlements().isSettlement(key)) return;
